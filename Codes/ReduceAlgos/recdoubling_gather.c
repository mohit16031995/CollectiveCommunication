...
//	int ready[CHUNK];
	...
	for (i=0;i<RUNS;i++)
	{
		...
		t1 = MPI_Wtime();
ifroot if(i>=82) printf("all ok 0\n");
		for (j=0; j<rounds; j++)
		{
			MPI_Irecv (data[recvatindex[j]],SIZE<<j,MPI_INT,partner[j],j,MPI_COMM_WORLD,&req[j]);
			//printf("rank %d to recv %d elements from %d at index %d\n",rank,SIZE<<j,partner[j],recvatindex[j]);
		}
		MPI_Isend (data[collected],SIZE,MPI_INT,partner[0],0,MPI_COMM_WORLD,&sreq[count++]);
		roundsdone=1;
ifroot if(i>=82) printf("all ok 1\n");
		while (roundsdone<rounds)
		{
ifroot if(i>=82) printf("all ok 2\n");
		...
		/*	if(thisround == MPI_UNDEFINED) 
			...
			MPI_Isend (data[collected],SIZE<<(thisround+1),MPI_INT,partner[thisround+1],thisround+1,MPI_COMM_WORLD,&sreq[count++]);
			//printf("rank %d to send %d elements to %d at index %d\n",rank,SIZE<<(thisround+1),partner[thisround+1],collected);
			roundsdone++;
ifroot if(i>=82) printf("all ok 3\n");
		}		
