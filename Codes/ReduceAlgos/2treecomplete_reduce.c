	...
	for (i=0;i<RUNS;i++)
	{
		...
		if(rank!=root) 
		{
			...
			{		
				...
				printf ("\trank %d received chunk %ld (actual %d)\n",rank,logical_chunk_no,index);
				for (k=logical_chunk_no*CSIZE,j=index*CSIZE;k<(logical_chunk_no+1)*CSIZE;k++,j++) 
				{
				...
				{	//forward this chunk ;
					printf("chunk no. %ld is ready to forward from rank %d\n",logical_chunk_no,rank);
			...
			{		
				...
				for (k=logical_chunk_no,j=index*CSIZE;k<logical_chunk_no+CSIZE;k++,j++) 
				{
					selfmsg[k] = (msg1[j]%100+selfmsg[k]%100)%100;
				}
				cdone++;
