...
// RUN MPI
	...
	// for send
	int rreq;
	MPI_Status sstt[CHUNK*2], rstt[CHUNK];
    ...
    for(i=0;i<RUNS;i++){
        ...
        if (rank == 0){  // if root then setup all send's
			...
			// left tree
                if(!(j%2)) {
                    MPI_Send(outmsg+j*CSIZE,CSIZE,MPI_CHAR, root->process_id,j,MPI_COMM_WORLD);
                    sent++;
			//timings[1][j][root->process_id] = MPI_Wtime()-t1;
                }
			// right tree
                else {
                    MPI_Send(outmsg+j*CSIZE,CSIZE,MPI_CHAR,root2->process_id,j,MPI_COMM_WORLD);
                    sent++;
		...
		MPI_Irecv(msg, CSIZE, MPI_CHAR, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &req[rreq++]);
		MPI_Irecv(msg+CSIZE, CSIZE, MPI_CHAR, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &req[rreq++]);
  //    	printf("Wait: rank = %d\n", rank);
	...
	//	printf("Recieved: Process id %d wait over recv %d chunk\n", rank, stt.MPI_TAG);
		...
		//timings[0][0][leftTreeNode[rank]->parent->process_id] = MPI_Wtime()-t1;
				...
				int recvToLeft = temp->parent->process_id;
				if (RUNS == 1)
		...
		//timings[0][1][rightTreeNode[rank]->parent->process_id] = MPI_Wtime()-t1;
				...
				int recvToRight = temp->parent->process_id;
				if (RUNS == 1)
		...
		//		printf("Process %d entered loop, turn = %d\n", rank, turn);
                ...
                if (leftrecvd != -2) {// left send
                    ...
                    if(temp->left_child != NULL && temp->leftColor == turn && leftleftsent <= leftrecvd/2) {
						...
						//checkflag = 1;
						sendFlag = 1;
						sendBuff = msg+leftrecvd*CSIZE;
						sendSize = CSIZE;
						sendTo = temp->left_child->process_id;
                        sendTag = leftrecvd;
                        sent++;
                    }
						...
						sendBuff = msg+leftrecvd*CSIZE;
						sendSize = CSIZE;
						sendTo = temp->right_child->process_id;
                        sendTag = leftrecvd;
                        sent++;
                    }
                }
		           ...
		           if (leftrecvd != -2) {
                       ...
                       if (j < CHUNK) {
                                ...
                                if (ierr == MPI_SUCCESS) {
                                    ...
                                    int recvToLeft = leftTreeNode[rank]->parent->process_id;
                                    if (RUNS == 1)
                                        printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, j, recvToLeft);
                                ...
                                checkflag = 1;*/
                                recvFlag = 1;
                                recvBuff = msg + j*CSIZE;
                                recvSize = CSIZE;
                                recvFrom = leftTreeNode[rank]->parent->process_id;
                                recvTag = j;
                                recieved += 1;
                                leftrecvd = j;
                        }
						...
						if (flag) {
							...
							int recvToLeft = leftTreeNode[rank]->parent->process_id;
							if (RUNS == 1)
								printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, 0, recvToLeft);
							//timings[0][0][leftTreeNode[rank]->parent->process_id] = MPI_Wtime()-t1;
                    ...
                    if (rightrecvd != -1) {
						...
						if (j < CHUNK) {
						    ...
						    if (ierr == MPI_SUCCESS) {
								...
								recieved += 1;
								rightrecvd = j;
								int recvToRight = rightTreeNode[rank]->parent->process_id;
								if (RUNS == 1)
									printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, j, recvToRight);
                            ...
                            recvBuff = msg + j*CSIZE;
                            recvSize = CSIZE;
                            recvFrom = rightTreeNode[rank]->parent->process_id;
                            recvTag = j;
                            recieved += 1;
                            rightrecvd = j;
						}
						...
						if (flag) {
							...
							int recvToRight = rightTreeNode[rank]->parent->process_id;
							if (RUNS == 1)
								printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, 1, recvToRight);
						}
                    }
                }

                if (sendFlag && recvFlag) {
                    MPI_Sendrecv(sendBuff, sendSize, MPI_CHAR, sendTo, sendTag, recvBuff, recvSize, MPI_CHAR, recvFrom, recvTag, MPI_COMM_WORLD, &stt);
                    if (RUNS == 1)
                        printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, recvTag, recvFrom);
                }
                else if (sendFlag) {
                    MPI_Send(sendBuff, sendSize, MPI_CHAR, sendTo, sendTag, MPI_COMM_WORLD);
                }
                else if (recvFlag) {
                    MPI_Recv(recvBuff, recvSize, MPI_CHAR, recvFrom, recvTag, MPI_COMM_WORLD, &stt);
                    if (RUNS == 1)
                        printf("Time %1.9lf rank %d recvd chunk %d from rank %d\n", MPI_Wtime()-t1, rank, recvTag, recvFrom);
                }
            }
        ...
        MPI_Waitall(rreq, req, rstt); // wait for all non-blocking recvs
	//printf("Computation\n");
		double total_test;
		t2 = MPI_Wtime() - t1;
