...
int main(int argc,char *argv[]){
	...
	if (rank != 0) {
		leftChildren = 0;
		rightChildren = 0;
		parentLeft = leftTreeNode[rank]->parent->process_id;
		parentRight = rightTreeNode[rank]->parent->process_id;
...
////		printf("My work is done here, rank %d\n",rank);
		...
		if(rank!=0) {// if not root setup all recvs
		   ...
		   for(j=0;j<CHUNK;j++) {
			// left tree				
			if(!(j%2)) { 
				MPI_Isend(selfmsg+j*CSIZE,CSIZE,MPI_INT,leftPeers[0],CHUNK+j,MPI_COMM_WORLD,&sreq[count++]);
			}			
			// right tree
			else { 
				MPI_Isend(selfmsg+j*CSIZE,CSIZE,MPI_INT,rightPeers[0],CHUNK+j,MPI_COMM_WORLD,&sreq[count++]);
			}			
		   }
		}						
