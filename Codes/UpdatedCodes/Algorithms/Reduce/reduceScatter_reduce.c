...
int main(int argc,char *argv[]){
	...
	for (int i=0;i<RUNS;i++)
	{
		...
		if(vrank != -1) {
			...
			while(mask < pof2) {
				if (vrank % mask != 0) {
				  ...
				  //MPI_Sendrecv(&reducedmsg[clen*send_start], vlen, MPI_INT, dest2, 1, &recvmsg[clen*recv_start], vlen, MPI_INT, dest2, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				  	MPI_Recv(&recvmsg[clen*recv_start], vlen, MPI_INT, dest2, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
						for (int ll = 0; ll < vlen; ll++) {
							reducedmsg[(clen*recv_start)+ll] = recvmsg[(clen*recv_start)+ll];
						}
					}
					else{
						MPI_Send(&reducedmsg[clen*send_start], vlen, MPI_INT, dest2, 1,MPI_COMM_WORLD);
					}
				  mask = mask << 1;
			}
		}
		...
		if (vrank == 0 && rank != 0) {
			MPI_Send(&reducedmsg, len, MPI_INT, 0, 1, MPI_COMM_WORLD);
		}
		if (rank == 0 && vrank != 0) {
			MPI_Recv(&reducedmsg, len, MPI_INT, 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		t2 = MPI_Wtime() - t1;
