 ...
 * Sparse AllReduce
 */
template<class IdxType, class ValType> int c_allreduce_small(const ValType *values, struct stream *recvbuf, unsigned dim, MPI_Comm comm) {
  ...
  if(vrank != -1) {
    ...
    // Split streams
    create_splitted_stream<IdxType, ValType>(values, splits, dim, pof2);
