 ...
 */
template<class IdxType, class ValType> int c_allreduce_recdoubling(const ValType *values, struct stream *recvbuf, unsigned dim, MPI_Comm comm) {
  ...
  if(worldsize == 1) {
    ...
    memcpy(recvbuf->items, values, dim * sizeof(ValType));
    return MPI_SUCCESS;
  }
  ...
  if (rank < 2 * rem) {
    ...
    if(rank % 2 == 0) {
      // Hack to created stream
      create_splitted_stream<IdxType, ValType>(values, &tmp1, dim, 1);
      MPI_Send(tmp1, countBytes<IdxType, ValType>(tmp1, dim), MPI_BYTE, rank+1, 1, comm);
      vrank = -1;
    } else {
      create_splitted_stream<IdxType, ValType>(values, &tmp2, dim, 1);
