...
template<class IdxType, class ValType> void create_splitted_stream(const fp_type  *values, struct stream **results, unsigned dim, unsigned worldsize) {
  ...
  for(size_t i = 0; i < worldsize; ++i) {
    ...
    // Count the number of elements in split
    unsigned tmp_cnt = 0;
    for(size_t j = offset; j < offset + maxsize; ++j) {
      if(fabs(values[j]) >= epsilon) {
        tmp_cnt++;
      }
    }
    ...
    if(dense) {
      for(size_t j = 0; j < maxsize; ++j) {
        ((ValType *)results[i]->items)[j] = values[j + offset];
      }
    } else {
      unsigned cnt = 0;
      // Copy values
      for(size_t j = 0; j < maxsize; ++j) {
        if(fabs(values[j + offset]) >= epsilon) {
          ((struct s_item<IdxType, ValType> *)results[i]->items)[cnt].idx = j;
          ((struct s_item<IdxType, ValType> *)results[i]->items)[cnt].val = values[j + offset];
          cnt++;
        }
      }
    }
