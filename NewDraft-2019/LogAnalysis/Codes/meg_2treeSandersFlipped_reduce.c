...
int main(int argc,char *argv[]){
	...
	if (rank != 0) {
		...
		//count no of childs
            ...
            if (t1->left_child != NULL) {
				...
				if (t1->right_child != NULL) {
					leftChildren = 1;
					leftPeers[0] = t1->right_child->process_id;
				}
			}
			...
			if (t1->left_child != NULL) {
				...
				if (t1->right_child != NULL) {
					rightChildren = 1;
					rightPeers[0] = t1->right_child->process_id;
				}
			}
	...
	printf("rank = %d, leftParent = %d, rightParent = %d, leftleftChild = %d, leftRightChild = %d, rightLeftChild = %d, rightRightChild = %d\n", rank, recvToLeft, recvToRight, leftPeers[0], leftPeers[1], rightPeers[0], rightPeers[1]);
...
////		printf ("%d leaf role completed\n",rank);
		...
		if(rank!=0) 
		{
			...
			{		
				...
				if (i == RUNS/2) {
					printf ("Log: Time %1.9f Rank %d received chunk %ld (actual %d)\n", MPI_Wtime()-t1,rank,logical_chunk_no,index);
				}
